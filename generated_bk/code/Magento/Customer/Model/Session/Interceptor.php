...
class Interceptor extends \Magento\Customer\Model\Session implements \Magento\Framework\Interception\InterceptorInterface
{
     ...
     * {@inheritdoc}
     */
    public function getAuthenticate($request)
    {
        $pluginInfo = $this->pluginList->getNext($this->subjectType, 'getAuthenticate');
        if (!$pluginInfo) {
            return parent::getAuthenticate($request);
        } else {
            return $this->___callPlugins('getAuthenticate', func_get_args(), $pluginInfo);
        }
    }
