...
class Config extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    public function getConfig($key, $storeId = null) {
        ...
        return $this->_coreHelper->getStoreConfig($key, $storeId);
    }
    ...
    {
        return [
            self::ACTIVE => $this->getActive($storeId),
            self::DATE => $this->getDate($storeId),
            self::DATE_FORMAT => $this->getDateFormat($storeId),
            self::DROPDOWN => $this->getDropdown($storeId),
            self::GMAP => $this->getDisplayGmap($storeId),
            self::HANDLING_FEE => $this->getHandlingFee($storeId),
            self::MINIMAL_DELAY => $this->getMinimalDelay($storeId),
            self::MINIMAL_DELAY_BACKORDER => $this->getMinimalDelayBackorder($storeId),
            self::SCHEDULE_AHEAD => $this->getScheduleAhead($storeId),
            self::TIME => $this->getTime($storeId),
            self::TIME_FORMAT => $this->getTimeFormat($storeId),
            self::TIME_RANGE => $this->getTimeRange($storeId),
            self::TITLE => $this->getTitle($storeId),
            self::GMAP_API_KEY => $this->getGmapAPIKey($storeId),
            self::DISPLAY_DESCRIPTION => $this->getDisplayDescription($storeId),
            self::CHOICE_TITLE => $this->getChoiceTitle($storeId),
            self::SHIPPING_ADDRESS_TITLE => $this->getShippingAddressTitle($storeId),
            self::PICKUPATSTORE_TITLE => $this->getPickupatstoreTitle($storeId),
